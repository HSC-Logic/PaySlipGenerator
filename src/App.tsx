...
import { WorkspaceTools, type WorkspaceToolId } from './components/common/WorkspaceTools'
...
import { chooseFolder, connectDrive, createGoogleDoc, disconnectDrive, driveConfigured, uploadPdfToDrive } from './services/googleDrive'
import { useAuth } from './features/auth/AuthProvider'
import { CloudSaveButton } from './components/payments/CloudSaveButton'
import { createBlankSlip, today } from './utils/paymentDefaults'
import { Link } from 'react-router-dom'

const blank = createBlankSlip
...
export default function App({ initialSlip, cloudRecordId = '', cloudCompanyId = '', showBranding = true }: { initialSlip?: PaymentSlip; cloudRecordId?: string; cloudCompanyId?: string; showBranding?: boolean } = {}) {
  const auth = useAuth()
  const [startup] = useState(() => initialSlip ? { slip: copySlip(initialSlip), baseline: copySlip(initialSlip), recovered: false, hasProfile: false, settings: loadSettings(localStorage) } : initialPaymentState())
  ...
  const [currentCloudRecordId, setCurrentCloudRecordId] = useState(cloudRecordId)
  const [reserveCloudReference, setReserveCloudReference] = useState(!cloudRecordId && !initialSlip)
  const cleanSlip = useRef<PaymentSlip>(copySlip(startup.baseline))
  const [darkMode, setDarkMode] = useState(() => startup.settings.theme === 'system' ? systemPrefersDark() : startup.settings.theme === 'dark')
  ...
  const [workspaceTool, setWorkspaceTool] = useState<WorkspaceToolId>('history')
  ...
  const clearAllStoredData = () => {
    ...
    setHistory([]); setHistoryQuery(''); setSavedRecipients([]); setHasCompanyProfile(false); setSettings(DEFAULT_SETTINGS); setDarkMode(systemPrefersDark()); acceptReplacement(blank(DEFAULT_SETTINGS.defaultCurrency)); tell('success', 'All Sliply data stored in this browser was cleared.')
  }
  ...
  const documentAction = (action: 'download' | 'print') => runDocumentAction({ action, slip, build: value => buildPdf(value, { showBranding }), download: downloadPdfDocument, print: printPdfDocument, setBusy, notify: tell, reportError: import.meta.env.DEV ? error => console.error(error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : { name: 'Unknown document error' }) : undefined })
  const savePdfToDrive = () => requireValid(() => void run('drive-pdf', async () => {
    let folderId = drive.folderId
    let folderName = drive.folderName
    if (!folderId) { const folder = await chooseFolder(); folderId = folder.id; folderName = folder.name }
    const file = await uploadPdfToDrive(slip, folderId)
    setDrive(current => ({ ...current, folderId, folderName, documentUrl: file.url }))
    tell('success', 'PDF saved to Google Drive.')
  }))
  const driveTools = <section className="drive-card" aria-label="Google Drive"><div className="drive-heading"><div className="drive-icon"><Cloud /></div><div><h2>Google Drive <span>Optional</span></h2><p>{!driveConfigured ? 'Google Drive is not configured for this deployment.' : drive.connected ? 'Connected for this browser session.' : 'Save an exact PDF or create an editable Google Doc.'}</p></div><span className={`status ${drive.connected ? 'connected' : ''}`}>{drive.connected ? 'Connected' : 'Not connected'}</span></div>
    {drive.folderName && <div className="folder-display"><HardDrive /> Saving to <strong>{drive.folderName}</strong></div>}
    <div className="inline-actions"><button className="button secondary" disabled={!driveConfigured || !!busy || drive.connected} onClick={() => run('connect', async () => { await connectDrive(); setDrive({ ...drive, connected: true }); tell('success', 'Google Drive connected for this session.') })}>{busy === 'connect' ? <LoaderCircle className="spin" /> : <PlugZap />} Connect Drive</button><button className="button secondary" disabled={!drive.connected || !!busy} onClick={() => run('folder', async () => { const f = await chooseFolder(); setDrive({ ...drive, folderId: f.id, folderName: f.name }) })}><FolderOpen /> Choose folder</button><button className="button secondary" disabled={!drive.connected || !drive.folderId || !valid || !!busy} onClick={() => run('doc', async () => { const doc = await createGoogleDoc(slip, drive.folderId); setDrive({ ...drive, documentUrl: doc.url }); tell('success', 'Google Doc created successfully.') })}>{busy === 'doc' ? <LoaderCircle className="spin" /> : <FilePlus2 />} Create Google Doc</button>{drive.connected && <button className="text-button" onClick={() => { disconnectDrive(); setDrive({ connected: false, folderId: '', folderName: '', documentUrl: '' }) }}><Unplug /> Disconnect</button>}</div>{drive.documentUrl && <a className="doc-link" href={drive.documentUrl} target="_blank" rel="noreferrer">Open saved Drive file ↗</a>}
  </section>
  return <div className="app-shell">
    <header className="app-header"><div className="app-brand"><div className="brand-mark"><FilePlus2 /></div><div><strong>Sliply</strong><span>Payment Slip Generator</span></div></div><div className="header-status"><Link className="header-account" to="/plans">Plans</Link>{auth.user?<Link className="header-account" to="/dashboard">Dashboard</Link>:<Link className="header-account" to="/login">Sign in</Link>}<button className="theme-toggle" onClick={toggleDarkMode} aria-label={`Switch to ${darkMode ? 'light' : 'dark'} mode`} title={`Switch to ${darkMode ? 'light' : 'dark'} mode`}>{darkMode ? <Sun /> : <Moon />}<span>{darkMode ? 'Light mode' : 'Dark mode'}</span></button><div className="privacy-pill"><ShieldCheck /> Browser-based</div></div></header>
    <main><section className="workspace-intro"><div><span className="eyebrow">DOCUMENT WORKSPACE</span><h1>{creationMode === 'quick' ? 'Create a quick payment slip' : 'Create a payment slip'}</h1><p>{creationMode === 'quick' ? 'Enter the essentials now and open advanced options only when needed.' : 'Complete four clear steps. Your information stays in place as you move between them.'}</p></div><div className="header-actions"><button className="button secondary" onClick={loadDraft}><FolderOpen /> Load draft</button><button className="button secondary" onClick={saveDraft}><Save /> Save draft</button><button className="button secondary" onClick={saveRecord}><Save /> {activeRecordId ? 'Update record' : 'Save to history'}</button><button className="button danger-subtle" onClick={clear}><RotateCcw /> Clear</button></div></section>
      <div className="creation-mode" role="group" aria-label="Payment creation mode"><button type="button" aria-pressed={creationMode === 'quick'} className={creationMode === 'quick' ? 'active' : ''} onClick={() => setCreationMode('quick')}>Quick Slip</button><button type="button" aria-pressed={creationMode === 'full'} className={creationMode === 'full' ? 'active' : ''} onClick={() => setCreationMode('full')}>Full details</button></div>
      <WorkspaceTools active={workspaceTool} onChange={setWorkspaceTool} panels={{
        history: <PaymentHistory records={filteredHistory} query={historyQuery} onQuery={setHistoryQuery} onLoad={editRecord} onDuplicate={duplicateRecord} onDelete={deleteRecord} />,
        drive: driveTools,
        settings: <SettingsPanel settings={settings} onSave={updateSettings} />,
        privacy: <PrivacyControls onClearDraft={clearStoredDraft} onClearHistory={clearStoredHistory} onClearRecipients={clearStoredRecipients} onClearReferences={clearStoredReferences} onClearAll={clearAllStoredData} />,
      }} />
      ...
      <div className={`workspace mobile-${mobileView}`}><div className="form-pane">{activeStep === 'review' ? <ReviewPanel slip={slip} errors={allErrors} /> : <SlipForm slip={slip} errors={visibleErrors} step={activeStep} quickMode={creationMode === 'quick'} hasCompanyProfile={hasCompanyProfile} savedRecipients={savedRecipients} selectedRecipientId={selectedRecipientId} onChange={setSlip} onSubmit={nextStep} onReference={() => setSlip({ ...slip, payment: { ...slip.payment, reference: generateReference({ prefix: settings.referencePrefix, existingReferences: persistedReferences(), onPersistenceFailure: recordBackgroundPersistence }) } })} onSaveCompany={saveCompany} onClearCompany={clearCompany} onSelectRecipient={selectRecipient} onSaveRecipient={saveRecipient} onDeleteRecipient={deleteRecipient} onLogoError={x => tell('error', x)} />}
        ...
        {storageWarning && <p className="storage-warning" role="status">{storageWarning}</p>}<p className="privacy-note"><LockKeyhole /> Stored locally by default. Drive actions run only after you connect.</p><div className="developer-mark">Developed by <a href="https://hsclogic.com/" target="_blank" rel="noreferrer">HSC Logic</a></div></div>
        <aside className="preview-pane" ref={preview}><div className="preview-toolbar"><div><Eye /><span>Live preview</span><i>{slip.payment.paperSize.toUpperCase()} · {slip.payment.orientation === 'portrait' ? 'Portrait' : 'Landscape'}</i></div><div className="total-chip"><span>Total</span><strong>{formatCurrency(total, slip.payment.currency)}</strong></div></div><div className="paper-wrap"><PaymentSlipPreview slip={slip} showBranding={showBranding} /></div></aside></div>
    </main>
    {activeStep === 'review' && <div className="action-dock"><div><span>Subtotal</span><strong>{formatCurrency(calculation.subtotal, slip.payment.currency)}</strong></div>{auth.user&&<CloudSaveButton slip={slip} recordId={currentCloudRecordId} preferredCompanyId={cloudCompanyId} reserveReference={reserveCloudReference} onSaved={(id, reference) => { setCurrentCloudRecordId(id); setReserveCloudReference(false); setSlip(current => ({ ...current, payment: { ...current.payment, reference } })) }}/>}<button className="button secondary" onClick={showPreview}><Eye /> Preview</button><button className="button secondary" disabled={!valid || !!busy} onClick={() => requireValid(() => void documentAction('print'))}>{busy === 'print' ? <LoaderCircle className="spin" /> : <Printer />} Print</button><button className="button primary" disabled={!valid || !!busy} onClick={() => requireValid(() => void documentAction('download'))}>{busy === 'download' ? <LoaderCircle className="spin" /> : <Download />} Generate PDF</button>{drive.connected && <button className="button secondary" disabled={!valid || !!busy} onClick={savePdfToDrive}>{busy === 'drive-pdf' ? <LoaderCircle className="spin" /> : <HardDrive />} Save to Drive</button>}<button className="button secondary" onClick={similar}><CopyPlus /> Create similar slip</button><button className="button secondary" onClick={another}><FilePlus2 /> New blank slip</button></div>}
