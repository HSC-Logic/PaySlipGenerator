...
import { clearAllSliplyData, clearCompanyProfile, clearDraftData, clearHistoryData, clearRecipientData, clearRecovery, clearReferenceData, loadCompanyProfile, loadDraft as loadStoredDraft, loadHistory, loadRecovery, loadRecipients, loadTheme, persistenceMessage, saveCompany as saveStoredCompany, saveDraft as saveStoredDraft, saveHistory as saveStoredHistory, saveRecipients as saveStoredRecipients, saveTheme, type PersistenceResult } from './utils/storage'
...
const initialPaymentState = () => {
  ...
  return recovery
    ? { slip: recovery.slip, baseline: recovery.baseline, recovered: true, hasProfile: profile !== null }
    : { slip: initial, baseline: initial, recovered: false, hasProfile: profile !== null }
}
...
export default function App() {
  ...
  const toggleDarkMode = () => { const next = !darkMode; setDarkMode(next); recordBackgroundPersistence(saveTheme(localStorage, next ? 'dark' : 'light')) }
  ...
  const clearAllStoredData = () => {
    ...
    setHistory([]); setHistoryQuery(''); setSavedRecipients([]); setHasCompanyProfile(false); setDarkMode(window.matchMedia('(prefers-color-scheme: dark)').matches); acceptReplacement(blank()); tell('success', 'All Sliply data stored in this browser was cleared.')
  }
